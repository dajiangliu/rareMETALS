...
extern "C" {
  ...
  double pMixChisq(double stat,std::vector<double>& lambda)
  {    
    std::vector<double> delta(lambda.size(),0.0),trace(7,0.0);
    std::vector<int> h(lambda.size(),1);
    int lim=10000,ifault=0,ql=1,r=lambda.size();
    double res=0.0,sigma=0.0,acc=0.0001;

    qfc1(&(lambda[0]),(&delta[0]),&(h[0]),&r,&sigma,&stat,&lim,&acc,&(trace[0]),&ifault,&res,&ql);
    return (1.0-res); 
  }
  ...
  void qfc1(double* lambda, double* delta, int* h, int *r, double *sigma, double *q, int *lim, double *acc, double* trace, int* ifault, double *res, int* ql)
  {
    for(int ii=0;ii<ql[0];ii++)
      {
	qfc(lambda,delta,h,r,sigma,&(q[ii]),lim,acc,trace,ifault,&res[ii]);
      }  
  }
